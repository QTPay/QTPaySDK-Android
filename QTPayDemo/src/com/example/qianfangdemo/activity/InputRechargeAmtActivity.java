...
public class InputRechargeAmtActivity extends BaseActivity {
	...
	private void getSettingConfig() {
		...
		mqt.getSettingConfiguration(new QTCallBack() {
			...
			public void onError(Map<String, String> errorInfo) {
				...
				AlertDialog dialog = new AlertDialog.Builder(InputRechargeAmtActivity.this).setMessage("获取请求参数失败").setPositiveButton("退出", new OnClickListener() {
					@Override
	...
	private void doJump() {
		...
		intent.putExtra(QTConst.EXTRO, new Gson().toJson(new QTHolder(QTConst.ServerType_RECHARGE, rechargeAmt, goods, extraInfo)));
